    ...
    <div className="glass-panel accent-purple h-full flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[11px] tracking-[0.15em] text-[#a1a1aa] font-light">Tracker</span>
        </div>
      ...
      <div className="mb-5">
        ...
        <div className="readout-grid">
          ...
          <div className="text-[9px] text-[#3f3f46] mt-3 font-light tracking-wider">27,600 km/h orbital velocity</div>
        </div>
      </div>

      <div className="h-px bg-[#27272a] mb-4"></div>
        ...
        <div className="space-y-3">
          {Object.entries(crypto).map(([symbol, data]) => (
            <div key={symbol} className="flex items-center justify-between py-1">
              <span className="text-[11px] text-[#a1a1aa] font-light">{symbol}</span>
              <div className="flex items-center gap-3">
                <span className="text-[12px] font-mono text-[#f4f4f5] tabular-nums">{formatPrice(data.price)}</span>
          ...
          {Object.keys(crypto).length === 0 && (
            <div className="text-[11px] text-[#71717a] animate-pulse font-light tracking-wider">Loading markets...</div>
          )}
        </div>
      </div>
    </div>
